...
class ReqUpdater:
    def __init__(self, requpconfig, final_msg):
        self.config = requpconfig
        self.final_msg = final_msg
    
    def process(self):
        try:
            ret = FirmsPublisher (config).publish (self.final_msg)
            if ret:
                return True
    ...
    try:
        ...
        if ret_reqUpdator_upd:
            ...
            try:
                ...
                print ('{} is publisehd: {}'.format (final_msg, ret))
            finally:
                print ("sending ack to the req updated queue ")
                return True
        else:
            return False
    except Exception as e:
